...
void DallasComponent::dump_config() {
  ...
  LOG_PIN("  Input pin: ", this->one_wire_->get_input());
  LOG_PIN("  Output pin: ", this->one_wire_->get_output());
...
DallasComponent::DallasComponent(ShellyOneWire *one_wire) : one_wire_(one_wire) {}
...
bool ICACHE_RAM_ATTR DallasTemperatureSensor::read_scratch_pad() {
  ShellyOneWire *wire = this->parent_->one_wire_;
  if (!wire->reset()) {
    return false;
  }
